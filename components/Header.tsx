    ...
    <header className="site-header sticky top-0 z-50 w-full max-w-full overflow-x-clip border-b border-line bg-background/95 backdrop-blur-md">
      <div className="hidden border-b border-line bg-surface lg:block">
        <div className="container-site flex flex-wrap items-center justify-between gap-3 py-2 text-sm text-muted">
          <p className="min-w-0 font-medium tracking-wide">
          ...
          <div className="flex flex-wrap items-center gap-3 xl:gap-4">
            ...
            <a
              ...
              className="inline-flex min-h-11 items-center font-display text-lg tracking-wide text-foreground transition hover:text-yellow"
            >
      ...
      <div className="container-site flex w-full max-w-full items-center gap-2 py-2.5 sm:gap-3">
        <Link
          href={localizedPath(locale)}
          className="group flex min-w-0 flex-1 items-center gap-2 md:flex-none md:max-w-[40%] lg:max-w-none"
        ...
        <nav className="hidden min-w-0 flex-1 items-center justify-end gap-2.5 md:flex lg:gap-4 xl:gap-5">
          {navKeys.map((item) => {
            ...
            return (
              <Link
                key={item.key}
                href={href}
                className={`inline-flex min-h-11 shrink-0 items-center whitespace-nowrap text-[0.7rem] font-semibold uppercase tracking-[0.1em] transition lg:text-sm lg:tracking-[0.12em] ${
        ...
        <button
          ...
          className="hamburger-btn inline-flex md:hidden"
          onClick={() => setOpen((v) => !v)}
        >
      ...
      {open ? (
        <div
          id="mobile-nav"
          className="mobile-nav border-t border-line bg-surface md:hidden"
        >
          <nav className="container-site flex flex-col gap-1 py-4">
            ...
            <a
              href={`tel:${siteConfig.phoneTel}`}
              className="inline-flex min-h-11 items-center rounded-sm px-3 py-3 text-sm font-semibold uppercase tracking-[0.14em] text-yellow"
              onClick={() => setOpen(false)}
            >
            ...
            {navKeys.map((item) => (
              <Link
                ...
                onClick={() => setOpen(false)}
                className="inline-flex min-h-11 items-center rounded-sm px-3 py-3 text-sm font-semibold uppercase tracking-[0.14em] text-foreground hover:bg-surface-2 hover:text-yellow"
              >
                {dict.nav[item.key]}
              </Link>
            ))}
